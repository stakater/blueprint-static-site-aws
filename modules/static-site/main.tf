module "s3bucket_host" {
  source = "github.com/stakater/blueprint-storage-aws.git//modules/s3/public/host?ref=v3.0.0"
  name = "${format("%s%s", "www.", var.domain_name)}"
  acl = "${var.bucket_acl}"
  policy = "${var.bucket_policy}"
}
module "s3bucket_redirect" {
  source = "github.com/stakater/blueprint-storage-aws.git//modules/s3/public/redirect?ref=v3.0.0"
  name = "${var.domain_name}"
  acl = "${var.bucket_acl}"
  redirect_all_requests_to = "${module.s3bucket_host.bucket_name}"
}
module "cloudfront_s3" {
    source = "github.com/stakater/blueprint-utilities-aws.git//modules/cloudfront?ref=v0.3.0"
...
module "route53_record_redirect_bucket" {
  source = "github.com/stakater/blueprint-utilities-aws.git//modules/route53/record/alias?ref=v0.3.0"
